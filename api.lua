function addSkin(name, origin, info, credits, model)
	local info = info
...
function editSkin(skin, name, origin, info, credits, model)
	local id = 0
	for i = 2, #skins do
...
local function deluxetohss(name, description, credit, color, modelInfo, forceChar, lifeIcon, camScale)
    addSkin(name, 'Character Select Coop', description, credit, modelInfo)
    return #skins
end
local function nono()
    return ":)"
